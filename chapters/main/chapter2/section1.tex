...
\end{verbatim}
    ...
    \begin{itemize}
        \item \textbf{\texttt{app.js}}: Là điểm nhập (Entry Point) duy nhất của hệ thống Backend. Tệp này có nhiệm vụ khởi tạo ứng dụng Express, thiết lập các middleware toàn cục (CORS, Body-parser), đăng ký hệ thống route và thiết lập kết nối đến máy chủ cơ sở dữ liệu.
        \item \textbf{\texttt{.env}}: Tệp lưu trữ các tham số môi trường dưới dạng key-value. Việc không lưu trực tiếp các bí mật (API Key, Database URL) vào mã nguồn giúp ngăn ngừa rò rỉ thông tin quan trọng khi quản lý qua Git.
    \end{itemize}
